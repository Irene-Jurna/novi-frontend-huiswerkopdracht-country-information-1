...
async function searchCountries(name) {
    try {
        const output = await (0, _axiosDefault.default).get(`https://restcountries.com/v2/name/${name}`);
        const country = output.data[0];
        console.log(country);
        countryData.innerHTML = `<span> 
        ...
        countryData.innerHTML = `<p>Error. ${name} is een onbekend land. Vul een geldige zoekopdracht in. </p>`;
    }
}
function countryCurrencies(currencies) {
    let result = "and you can pay with ";
    if (currencies.length === 2) return result + `${currencies[0].name} and ${currencies[1].name}`;
...
function countryLanguages(languages) {
    ...
    for(let i = 0; i < languages.length; i++){
        ...
        if (i === languages.length - 1) ret += " and ";
        else if (i > 0) ret += ", ";
        ret += languages[i].name;
    }
    return ret;
